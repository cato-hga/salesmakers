...
gem 'browser-timezone-rails', git: 'https://github.com/kbaum/browser-timezone-rails.git' #using the timezone of a user
gem 'capistrano', '3.4.0' # Automated deployment
gem 'capistrano-bundler', '1.1.4' # Capistrano bundler integration
gem 'capistrano-rails', '1.1.3' # Capistrano Rails integration
...
gem 'griddler', '1.2.1' # Incoming email parsing
gem 'griddler-postmark', '1.0.0' # Adapter for griddler for postmark
gem 'groupdate', '2.4.0' # Ability to group by dates
gem 'haml-rails', '0.9.0' # HAML template engine
gem 'httparty', '0.13.5' # HTTP REST API client
gem 'jbuilder', '2.3.1'
...
gem 'net-sftp', '2.1.2' # SFTP client
...
gem 'postmark-rails', '0.11.0' # Postmark integration. Currently for contact/feedback form
...
gem 'sidekiq', '3.4.2' # Background job queuing system
...
group :development do
  ...
  gem 'coderay', '1.1.0'
  gem 'coolline', '0.5.0'
  gem 'guard-rspec', '4.6.4', require: false # Automated spec runs
  ...
  gem 'spring', '1.3.6' # Rails environment preloading
  gem 'traceroute', '0.5.0' # Find dead routes and missing controller actions.
  gem 'web-console', '~> 2.0' # Web console (new to rails 4.2)
end

group :test do
  gem 'capybara', '2.4.4' # Testing views and interactions
  gem 'capybara-webkit', '1.6.0' # Javascript driver for capybara
  gem 'codeclimate-test-reporter', '0.4.7', require: nil # Code Climate test coverage
  gem 'database_rewinder', '0.5.3' # Database Cleaner alternative
...
group :development, :test do
  ...
  gem 'rspec-rails', '3.3.3' # Rspec test framework
end
