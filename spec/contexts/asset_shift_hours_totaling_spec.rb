...
describe AssetShiftHoursTotaling do
  ...
  describe '.email_managers' do
    let(:person_with_supervisor) { create :person, supervisor: supervisor }
    let(:second_person_with_supervisor) { create :person, supervisor: supervisor }
    let(:supervisor) { create :person }
    let(:no_supervisor) { create :person, supervisor: nil }
    let(:people) { [person_with_supervisor, no_supervisor] }
    let(:orphaned_person) { create :person, supervisor: nil }

    before {
      @total = AssetShiftHoursTotaling.new(duration)
    }

    it 'emails managers for all employees in the array generated by .generate_totals', pending: 'need a new person method' do
      expect(AssetsMailer).to receive(:asset_approval_mailer).with(supervisor)
    end
    it 'does not email managers twice if they have two people who passed 40 hours', pending: 'need a new person method' do
      expect(AssetsMailer).to receive(:asset_approval_mailer).with(supervisor).once
    end
    it 'emails the area manager of territory if there is not a direct supervisor' do

    end
    it 'emails the regional manager if there is not an area manager'
    it 'emails IT if there is not a manager, area manager, or regional for an employee'
    #make a new mailer
  end
end