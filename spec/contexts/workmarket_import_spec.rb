...
describe WorkmarketImport do
  ...
  it 'sets locations', vcr: { record: :once, match_requests_on: [:body] } do
    expect(import).to receive(:set_locations)
    import.execute
  end

  it 'filters the locations to only unsaved', vcr: { record: :once, match_requests_on: [:body] } do
    expect(import).to receive(:filter_to_new_locations)
    import.execute
  end

  it 'saves locations', vcr: { record: :once, match_requests_on: [:body] } do
    expect {
      import.execute
    }.to change(WorkmarketLocation, :count).by_at_least(1)
  end

  it 'sets completed assignments', vcr: { record: :once, match_requests_on: [:body] } do
    expect(import).to receive(:set_completed_assignments)
    import.execute
  end

  it 'deletes the assignments before importing', vcr: { record: :once, match_requests_on: [:body] } do
