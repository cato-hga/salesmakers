...
describe AssetsMailer do
  ...
  describe '.asset_approval_mailer' do
    ...
    it 'sends an email with correct subject' do
      expect(mail.subject).to include('New Employees to Approve for Tablet Deployment')
    end
    ...
    it 'sends an email with the link to newcenter ' do
      expect(mail.body.encoded).to have_content('Please visit http://newcenter.salesmakersinc.com/people/tablet_approval')
      expect(mail.body.encoded).to have_content('and click the "Tablet Approval" link')
    end
  end
end