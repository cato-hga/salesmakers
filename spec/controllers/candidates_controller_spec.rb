...
describe CandidatesController do
  ...
  describe 'GET welcome_call' do
    let(:candidate) { create :candidate }
    before(:each) do
      allow(controller).to receive(:policy).and_return double(welcome_call?: true)
    end
    it 'returns a success status' do
      get :welcome_call, id: candidate.id
      expect(response).to be_success
      expect(response).to render_template(:welcome_call)
    end
  end

  describe 'POST record_welcome_call' do
