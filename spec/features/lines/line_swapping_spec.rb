...
describe 'Swapping lines' do
  ...
  describe 'when both lines have devices' do
    ...
    let!(:line) { create :line, identifier: '7274985180', line_states: [line_state] }
    let!(:second_line) { create :line, identifier: '7274985181', line_states: [line_state] }
    let!(:device_two) { create :device, serial: '998512', identifier: '998512', line: second_line }
    ...
    describe 'the swap results page' do
      it 'DOES NOT prompt the user to pick between deactivated the swapped line or keeping it active' do
        expect(page).to have_content('Swap Results')
        expect(page).not_to have_content("Do you want to deactivate the device's original line or keep it active?")
      end
    ...
    describe 'submission success' do
      before(:each) do
        within('#swap_results_two_lines') do
          click_on('Confirm Swap')
        end
      end
      it 'shows a confirmation' do
      ...
      it 'creates log entries' do
        ...
        within('.history') do
          expect(page).to have_content "Swapped the device's line from #{second_line.identifier}"
        end
      end
    end
