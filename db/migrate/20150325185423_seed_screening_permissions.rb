class SeedScreeningPermissions < ActiveRecord::Migration
  def self.up
    permission_group = PermissionGroup.find_or_create_by name: 'People'
    permission_update = Permission.find_or_create_by key: 'screening_update',
                                                     description: "Can add and edit employee screening info",
  ...
  def self.down
    ...
    permission_update = Permission.find_or_create_by key: 'screening_update',
                                                     ...
                                                     permission_group: permission_group
    for position in get_positions do
      position.permissions.delete permission_update
    end
  end
