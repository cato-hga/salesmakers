class SeedCandidateViewAllPermission < ActiveRecord::Migration
  def self.up
    permission_group = PermissionGroup.find_or_create_by name: 'Candidates'
    permission = Permission.find_or_create_by key: 'candidate_view_all',
  ...
  def self.down
    ...
    Permission.where(key: 'candidate_view_all').destroy_all
  end
