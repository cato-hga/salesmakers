class SeedOutsourcedDoorsPermissions < ActiveRecord::Migration
  def self.up
    permission_group = PermissionGroup.find_or_create_by name: 'Areas and Lcoations'
    ...
    for position in get_positions do
      position.permissions << permission_outsourced
    end
  end
  ...
  def self.down
    permission_outsourced = Permission.find_or_create_by key: 'location_area_outsourced',
                                                         ...
                                                         permission_group: permission_group
    for position in get_positions do
      position.permissions.delete permission_outsourced
    end
  end
